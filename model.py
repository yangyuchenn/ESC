...
class SGRAF(object):
    ...
    def train(
        ...
        if mode == "eval_loss":
            ...
            ## ESC regularization
            sim = sims.diag()
            clean_index = sim.topk(k=1, dim=-1, largest=True)[1]
            indices = torch.ones_like(sim)
            indices[clean_index] = 0.
            image_c = img_embs.index_select(0, clean_index.cuda())
            text_c = cap_embs.index_select(0, clean_index.cuda())

            image_anchor = image_c.squeeze(dim=0).expand_as(img_embs)
            text_anchor = text_c.squeeze(dim=0).expand_as(cap_embs)
            text_lens = [cap_lens[clean_index]]*len(cap_lens)

            sim1 = self.forward_sim(image_anchor, cap_embs, cap_lens)
            sim2 = self.forward_sim(img_embs, text_anchor, text_lens)

            loss_eq = torch.pow(l2norm(sim1 - sim2), 2).diag()
            # margin = loss_eq[clean_index].mean()

            return loss + 0.5 * loss_eq
    ...
    def train_eq(
        ...
        img_embs, cap_embs, cap_lens = self.forward_emb(images_l, text_l, lengths_l)
        sim = self.forward_sim(img_embs, cap_embs, cap_lens)
        ...
        loss_eq = torch.pow(l2norm(sim1 - sim2), 2).diag().sum()
